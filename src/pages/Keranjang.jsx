...
const initialCart = [
  { id: "1", emoji: "🍢", name: "Tahu Bakso", price: 12500, qty: 2 },
  { id: "2", emoji: "🌈", name: "Bolu Gulung Pelangi", price: 5000, qty: 3 },
  { id: "3", emoji: "🥟", name: "Pastel", price: 2500, qty: 1 },
];

export default function Keranjang({ onNavigate }) {
  const navigate = useNavigate();
  const [items, setItems] = useState(initialCart);
  const [selectedIds, setSelectedIds] = useState(initialCart.map((item) => item.id));
      ...
      <div className="px-4 lg:px-8 py-5 space-y-5">
        <div className="rounded-3xl bg-white/40 border border-pink-100 p-4 shadow-card">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm font-bold text-text-dark">Pilih item untuk checkout</p>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <p className="text-sm font-bold text-pink-6">{formatRp(item.price)}</p>
                  </div>
                  <p className="text-xs text-text-mid mt-1">Jumlah: {item.qty}</p>
                  <p className="text-xs text-text-mid mt-2">
                    Total: <span className="font-semibold text-text-dark">{formatRp(item.qty * item.price)}</span>
                  </p>
                </div>
              </div>
            );
      ...
      <div className="lg:hidden fixed bottom-20 left-0 right-0 z-40 bg-white/95 border-t border-pink-200 px-4 py-4 backdrop-blur-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="font-bold text-text-dark">{formatRp(totalSelected)}</p>
          </div>
          <button
            onClick={handleCheckout}
