...
const notifikasiData = [
  ...
  { id: 2, judul: "Risol Mayo Sedang Dikirim", deskripsi: "Pesananmu sedang dalam perjalanan, tunggu sebentar lagi ya!", waktu: "15 menit lalu", warna: "bg-yellow-400", tipe: "Pesan", aksi: null },
  { id: 3, judul: "Pesanan Telah Selesai!", deskripsi: "Tahu Baksomu telah sukses diantar. Yuk beri rating!", waktu: "Kemarin", warna: "bg-green-500", tipe: "Pesan", aksi: "Beri Rating" },
  { id: 4, judul: "Promo Gratis Ongkir Minggu Ini!", deskripsi: "Nikmati gratis ongkir untuk semua pesanan minggu ini. S&K berlaku.", waktu: "2 hari lalu", warna: "bg-pink-4", tipe: "Promo", aksi: null },
  { id: 5, judul: "Order #MPX28752 Telah Dibatalkan", deskripsi: "Pesanan Sosis Solomu telah dibatalkan. Yuk coba pesan lagi!", waktu: "2 minggu lalu", warna: "bg-pink-6", tipe: "Pesan", aksi: null },
];
  ...
  return (
    ...
    >
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`text-sm font-bold pb-1 relative transition-colors ${
              activeTab === t ? "text-pink-6" : "text-text-mid hover:text-text-dark"
            }`}
          >
            {t}
            {activeTab === t && (
      ...
      <div className="flex flex-col gap-3 px-4 lg:px-8 py-4">
        {filtered.map((n) => (
          <div key={n.id} className="bg-white rounded-2xl p-4 shadow-card flex gap-3 items-start">
            <div className={`w-10 h-10 rounded-full ${n.warna} shrink-0 mt-0.5`} />
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 mb-1">
                <p className="text-sm font-extrabold text-text-dark leading-snug">{n.judul}</p>
                <span className="text-[10px] text-text-mid whitespace-nowrap shrink-0">{n.waktu}</span>
              </div>
              <p className="text-xs text-text-mid leading-relaxed">{n.deskripsi}</p>
              {n.aksi && (
