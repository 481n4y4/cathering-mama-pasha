...
export default function Sidebar({ activeMenu, onNavigate, children, title, onBack }) {
  ...
  const userData = rawUser ? (() => { try { return JSON.parse(rawUser); } catch { return null; } })() : null;
  const namaUser = userData?.nama_user || userData?.nama || "User";
  const profilePhoto =
    userData?.foto_profile || userData?.foto_profil || userData?.photo || logoMamaPasha;
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div
        ...
        style={{
          background: "linear-gradient(160deg, #FCC7D1 0%, #F0B3C5 50%, #E47990 100%)",
        }}
      >
        {/* ── Top bar MOBILE ── */}
